...
export const fetchGemini = async (payload, isJson = false) => {
  ...
  for (let i = 0; i < 5; i++) {
    try {
      console.log(`[fetchGemini] 시도 ${i+1}/5, isJson=${isJson}`);
      const response = await fetch(url, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body)
      });
      console.log(`[fetchGemini] 응답 status: ${response.status}`);
      if (!response.ok) {
        // 429 한도 초과 에러는 재시도 없이 즉시 안내 메시지 throw
        ...
        // 503 서버 과부하 에러는 재시도 없이 즉시 안내 메시지 throw
        ...
        console.error(`[fetchGemini] 응답 실패 body:`, errText);
        throw new Error(`API 응답 실패 (${response.status})`);
      }
      const data = await response.json();
      console.log(`[fetchGemini] 응답 data.success=${data.success}, data keys=${Object.keys(data).join(',')}`);
      
      // success: true 이거나 text가 있으면 성공으로 처리
      const responseText = data.text;
      if (!responseText && !data.success) {
        throw new Error(data.message || data.error || "서버 오류: 응답에 텍스트가 없습니다");
      }
      ...
      if (isJson) {
        try {
          ...
          console.error('[fetchGemini] JSON 파싱 실패:', parseErr, '원본 텍스트:', responseText?.substring(0, 200));
